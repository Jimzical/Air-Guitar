...
def DrawBoard(image, AnchorStartPoint,AnchorEndPoint , lines = 4,xoffset = 20,yoffset =20, offset = 0,dynamic = False,log = False , length = 2, thickness = 4):
    ...
    for stringLine in range(lines):
        ...
        if log:
            print("String {} => Start {} --> End {} StOffset ===> {}, off -) {}".format(stringLine, start, end,Stoffset,offset))
...
def main():
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            draw = DrawBoard(image, point["Wrist"], point["Middle MCP"], lines=3, offset=20,log=True , dynamic=False)
