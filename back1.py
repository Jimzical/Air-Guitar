...
# Current Working Version: 3.0
# Features:
# - Uses Index Finger for Strumming
# - Plays the String Sound when the Index Finger is near the String
# TODO:
# Re-Record the Sounds as the Starting Sounds are too Soft
# Setup Chord Detection Mechanism
# Setup Chords
# Fix the dynamic string offset to be more smooth
...
def initializeMusic():
    ...
    sound = { 0 : mixer.Sound('Media/'+stringToString[0]+'.wav'), 1 : mixer.Sound('Media/'+stringToString[1]+'.wav'), 2 : mixer.Sound('Media/'+stringToString[2]+'.wav'), 3 : mixer.Sound('Media/'+stringToString[3]+'.wav'), 4 : mixer.Sound('Media/'+stringToString[4]+'.wav'), 5 : mixer.Sound('Media/'+stringToString[5]+'.wav')}
    return sound
    # sound = {"A2" : mixer.Sound('Media/A2.wav'), "B3" : mixer.Sound('Media/B3.wav'), "D3" : mixer.Sound('Media/D3.wav'), "E2" : mixer.Sound('Media/E2.wav'), "E4" : mixer.Sound('Media/E4.wav'), "G3" : mixer.Sound('Media/G3.wav')}
...
def ContactCheck(image,draw, finger,sound,log = False, accuracy = 10, logSize = 0.75, logColor = (0, 255, 0)):
    '''
    -------------------------------------------------------------
...
def main():
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks and len(results.multi_hand_landmarks) == 2:
            ...
            # q: whats (100, 50) and 0.75?
            # a: (100, 50) is the position of the text and 0.75 is the size of the text
            ...
            # Removed Hand Marking For Now
            # markHands(image, results, left,mp_drawing,mp_drawing_styles,mp_hands)
            AddBlobToFinger(image, left["Index TIP"])
            # draw = DrawBoard(image, left["Wrist"], right["Index DIP"], lines=6,length=1, offset=2 0,log=False , dynamic=False, absolute_length=True ,absoulute_offset= 120 )
            
            draw = DrawBoard(image, right["Index MCP"], right["Pinky MCP"], lines=6,length=4, offset=20,log=False , dynamic=False , fretOffset=20)
            ContactCheck(image,draw, left["Index TIP"],sound=sound,log = False, accuracy = 5,logColor=color)
        
            
        elif results.multi_hand_landmarks:
            cv2.putText(image, "One Hand Detected Only", (100, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.75, color, 2)
        else:
            cv2.putText(image, "No Hands Detected", (100, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.75, color, 2)
        showImage(image)
